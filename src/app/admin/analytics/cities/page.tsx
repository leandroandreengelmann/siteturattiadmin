...
function CityDetail({
  ...
  useEffect(() => {
    const loadCityDetails = async () => {
      setLoading(true);
      try {
        const devices = await analyticsService.getDevicesByCity(selectedCity, startDate, endDate);
        const pageViews = await analyticsService.getPageViewsByCity(selectedCity, startDate, endDate);
...
// Componente para o mapa simples do Brasil com cidades
function BrazilMap({ 
  cityData, 
  onCitySelect 
}: { 
  ...
  // Simular posições relativas de algumas cidades principais no mapa do Brasil
  // Estas são posições aproximadas para um mapa estilizado
  const cityPositions: {[key: string]: {x: number, y: number}} = {
    'São Paulo': {x: 65, y: 70},
    'Rio de Janeiro': {x: 70, y: 65},
    'Belo Horizonte': {x: 65, y: 55},
    'Salvador': {x: 85, y: 45},
    'Brasília': {x: 60, y: 45},
    'Fortaleza': {x: 85, y: 25},
    'Recife': {x: 90, y: 35},
    'Porto Alegre': {x: 55, y: 85},
    'Curitiba': {x: 60, y: 75},
    'Manaus': {x: 35, y: 20},
    'Belém': {x: 60, y: 20},
    'Goiânia': {x: 55, y: 50},
    'Florianópolis': {x: 60, y: 80},
    'Vitória': {x: 75, y: 60},
    'Campo Grande': {x: 45, y: 60},
    'Cuiabá': {x: 45, y: 45},
    'João Pessoa': {x: 90, y: 30},
    'Maceió': {x: 88, y: 40},
    'Guarulhos': {x: 64, y: 69},
    'Campinas': {x: 62, y: 68},
  };
  ...
  return (
    <div className="relative bg-slate-100 rounded-xl overflow-hidden h-[500px] w-full">
      ...
      >
        <path d="M30,15 Q45,5 60,15 Q80,20 85,40 Q90,60 80,80 Q60,95 40,80 Q25,75 35,60 Q25,50 20,35 Q25,25 30,15" 
          strokeWidth="0.5"
        />
      </svg>
...
export default function CityAnalyticsPage() {
  ...
  const loadCityData = async () => {
    setLoading(true);
    try {
      const data = await analyticsService.getBrazilCityData(startDate, endDate);
      setCityData(data);
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="flex items-center mb-2">
          ...
          <h1 className="text-2xl font-bold text-slate-800">Análise por Cidades do Brasil</h1>
        </div>
        <p className="text-slate-500">Visualize métricas detalhadas por cidade</p>
      </div>
        ...
        <>
          {!selectedCity ? (
            <>
              ...
              <div className="bg-white rounded-xl shadow-sm p-6">
                <h2 className="text-lg font-semibold text-slate-800 mb-4">Distribuição de visitantes por cidade</h2>
              ...
              <div className="bg-white rounded-xl shadow-sm p-6">
                <h2 className="text-lg font-semibold text-slate-800 mb-4">Ranking de cidades</h2>
